...
export async function run(args: any) {
  ...
  const lspPath = join(
    extensions.getExtension("Sarrus.amxxpawn-vscode").extensionPath,
    "languageServer"
  );
  ...
  if (version === undefined) {
    window.showErrorMessage(
      "Failed to download the latest version of the AMXXPawn Language Server."
    );
    return 1;
  }
